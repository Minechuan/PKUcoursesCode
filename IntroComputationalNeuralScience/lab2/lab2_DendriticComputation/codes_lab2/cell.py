...
class HPC:
    def __init__(self, filepath,  spine_density):
    ...
    def setup_cell(self, filepath):
            ...
            morphology_file= filepath + '/morphologies/cell1.asc'
    ...
    def fill_clustered_synapses_list_with_spine(self, seglist, rd, config):
        ...
        for seg in seglist:
            ...
            for i in range(config.CLUSTER_SIZE):
                ...
                #syn_segments.append(sec(x))
                syn_seg = sec(x)
                if syn_seg not in self.spine_seg_list:
                    #print("no spine", syn_seg)
                    syn_segments.append(syn_seg)
                else:
                    #spine_list_id, nspine = self.map_seg2spine_listid[seg]
                    #neck, head = self.add_single_spine(seg)
                    neck, head = self.stim_on_spine(syn_seg)
                    syn_segments.append(head(config.SPINE_HEAD_X))
                    #self.map_seg2spine_listid[seg][1] += 1
