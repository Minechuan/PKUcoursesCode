...
def main():
    ...
    # list the parameters in the network
    ...
    save_mask_scores(net.state_dict(), os.path.join(args.output_dir, f'mask_values_{args.anp_alpha}.txt'))
...
def mask_train(model, criterion, mask_opt, noise_opt, data_loader):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        for _ in range(args.anp_steps):
            ...
            include_noise(model)
            output=model(images)
            loss_noise = - criterion(output, labels) # max loss
            loss_noise.backward()
            sign_grad(model)
            noise_opt.step()
            clip_noise(model,eps=args.anp_eps)

        mask_opt.zero_grad()
        # step 2: calculate noise loss 
        include_noise(model)
        output_n = model(images)
        loss_rob = criterion(output_n, labels)
        exclude_noise(model)
        # step 3: calculate clean loss
        output_c=model(images)
        loss_clean = criterion(output_c,labels)
        loss = args.anp_alpha*loss_clean+(1-args.anp_alpha)*loss_rob
        total_loss += loss.item()
        pred = output_c.argmax(dim=1)
