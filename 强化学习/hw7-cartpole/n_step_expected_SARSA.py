...
class DiscreteCartPoleEnv(gym.Env):
    def __init__(self, intervals=8):
        # support gym versions with/without render_mode arg
        try:
...
# -----------------------
# n-step Expected SARSA learner
# -----------------------
class NStepExpectedSarsaLearner:
    ...
    def policy_probs(self, state):
        """
        Return action probabilities under current epsilon-greedy policy at given state.
        pi(a*) = 1 - epsilon + epsilon/|A|
        others = epsilon/|A|
        """
        nA = self.env.action_space.n
        probs = np.ones(nA) * (self.epsilon / nA)
        best = int(np.argmax(self.q[state]))
        probs[best] += (1.0 - self.epsilon)
        return probs

    def expected_q(self, state):
        """
        Compute expected Q(s, ·) under current epsilon-greedy policy.
        """
        if state is None:
            return 0.0
        probs = self.policy_probs(state)
        return float(np.dot(probs, self.q[state]))
    ...
    def train(self):
        for ep in range(self.start_iter, self.iter + 1):
            # initialize lists per textbook indexing
            S = []
            A = []
            R = [0.0]  # R[0] dummy, use R[1] as reward after S0->S1

            s0 = self.env.reset()
            a0 = self.epsilon_greedy_action(s0)
            S.append(s0)
            A.append(a0)
            ...
            while True:
                ...
                if t < T:
                    ...
                    if done:
                        ...
                        a_tp1 = self.epsilon_greedy_action(s_tp1)
                        S.append(s_tp1)
                        A.append(a_tp1)
                ...
                tau = t - self.n + 1
                if tau >= 0:
                    # compute G: sum of discounted rewards from tau+1 to min(tau+n, T)
                    G = 0.0
                    upper = int(min(tau + self.n, T))
                    for i in range(tau + 1, upper + 1):
                        G += (self.gamma ** (i - (tau + 1))) * R[i]

                    # bootstrap with expected Q if tau + n < T
                    if tau + self.n < T:
                        s_tn = S[tau + self.n]
                        G += (self.gamma ** self.n) * self.expected_q(s_tn)
...
def train_test_model(n_step, intervals=8, episodes=200):
    ...
    save_path = f'q_tables_expected_sarsa_n{n_step}_i{intervals}'
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)

    learner = NStepExpectedSarsaLearner({
        'env': env,
        'test_env': test_env,
        'q': q_table,
        'n': n_step,
...
if __name__ == '__main__':
    ...
    plt.title('n-step Expected SARSA: mean reward per checkpoint')
    plt.legend()
    plt.grid(True)
    plt.show()
