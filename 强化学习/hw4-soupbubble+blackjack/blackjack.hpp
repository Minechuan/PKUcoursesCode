...
Blackjack::StepResult Blackjack::step(int action){
    ...
    if (action == HIT){  
        ...
        switch(_state.turn){
            case PLAYER: // player STICKs, then dealer's turn
                if (verbose){
                    cout << "Player stick at " << _state.player_sum << ".\nDealer's turn." << endl;
                }
                _state.turn = DEALER;
                return StepResult(_state, 0.0, 0.0, false);
            case DEALER: // dealer STICKs, then compare results
                if (verbose){
...
void Blackjack::deal_card(int card){ // Actually change state here
