    ...
    int next_cars_1, int next_cars_2, double &expected_reward) const
{
    ...
    // compute post-move overnight counts, then clamp to [0,MAX_IDX]
    int early_next_raw_1 = curr_cars_1 - action;
    int early_next_raw_2 = curr_cars_2 + action;
    int early_next_cars_1 = std::min(MAX_IDX, std::max(0, early_next_raw_1));
    int early_next_cars_2 = std::min(MAX_IDX, std::max(0, early_next_raw_2));
    ...
    // ---- compute move cost (with one free shuttle from loc1->loc2) ----
    ...
    // ---- compute parking penalty based on post-move overnight counts ----
    // If more than 10 cars are kept overnight at a location (after moving), add $4 per location
    ...
    for (int req1 = 0; req1 <= MAX_IDX; ++req1) {
        ...
        for (int req2 = 0; req2 <= MAX_IDX; ++req2) {
            ...
            // served count limited by the clamped early_next counts
            ...
            // reward: rental revenue minus move cost minus parking penalty
