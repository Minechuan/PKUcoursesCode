...
def MyCELoss(pred, gt):
    ...
    gt_one_hot = torch.zeros_like(pred)
    gt_one_hot.scatter_(1, gt.view(-1, 1), 1) # one-hot encoding
    loss = -torch.sum(gt_one_hot * torch.log(pred + 1e-8)) / pred.shape[0] # CE loss
    return loss 
...
# python .\train.py --exp_name hw2_runbs64 --batchsize 128 --lr 1e-3

# 完成自己的训练：
'''
# python .\train.py --exp_name lr1e-4_dataaug2 --batchsize 64 --lr 1e-4
# python .\train.py --exp_name lr1e-4_dataaug7 --batchsize 64 --lr 1e-4 --save_freq 3 --total_epoch 25
'''